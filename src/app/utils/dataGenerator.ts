...
export const generateBooks = (
    seed: number,
    language: "en-US" | "de-DE" | "ar", // Supported languages
    region: string,
    likes: number,
    reviews: number,
    page: number
    ...
    return Array.from({ length: 20 }, (_, i) => {
        // Generate a title dynamically with fallback for Arabic
        const wordCount = Math.floor(rng() * 5) + 1; // Random word count (1-5)
        let title: string;

        if (language === "ar") {
            // Special handling for Arabic using lorem.words()
            title = localizedFaker.lorem.words(wordCount);
        } else {
            // Use word.words() for other languages
            title = localizedFaker.word.words(wordCount);
        }
        ...
        return {
            index: i + 1 + (page - 1) * 20, // Sequential index
            isbn: localizedFaker.string.numeric(13), // 13-digit numeric ISBN
            title: capitalizeSentence(title), // Title with proper capitalization
            author, // Consistent, localized author
            publisher, // Consistent, localized publisher
            likes: Math.floor(rng() * likes), // Randomized likes
            reviews: Math.floor(rng() * reviews), // Randomized reviews
